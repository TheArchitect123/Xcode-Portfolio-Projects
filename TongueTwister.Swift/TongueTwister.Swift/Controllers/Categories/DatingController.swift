...
internal class DatingController : BaseTableViewController{
    let CustomSource : PhrasesDataSource = PhrasesDataSource.init(PhraseCategories.GeneralConversation);
       ...
       internal override func RefreshItems() {
           ...
           self.view.makeToast("Refreshing all \(FriendlyNames.GetCategoryForNames(PhraseCategories.Dating)) phrases. Please wait a few seconds...", duration: 3.0, position: .bottom)
           
           RestConsumerHelper.Get_DefaultRestConsumer()
               .resource("/get_phrases").withParam("category", "\(PhraseCategories.Dating.rawValue)").loadIfNeeded()?.onSuccess { entity in
