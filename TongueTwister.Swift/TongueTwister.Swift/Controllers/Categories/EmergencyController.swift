...
internal class EmergencyController : BaseTableViewController {
     ...
     internal override func RefreshItems() {
         ...
         self.view.makeToast("Refreshing all \(FriendlyNames.GetCategoryForNames(PhraseCategories.Emergency)) phrases. Please wait a few seconds...", duration: 3.0, position: .bottom)
         
         RestConsumerHelper.Get_DefaultRestConsumer()
             .resource("/get_phrases").withParam("category", "\(PhraseCategories.Emergency.rawValue)").loadIfNeeded()?.onSuccess { entity in
