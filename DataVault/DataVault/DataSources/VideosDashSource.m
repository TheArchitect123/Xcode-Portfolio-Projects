...
-(void) configureTableCell:(uint)index tableCell:(UITableViewCell**)cell{
    ...
    (*cell).detailTextLabel.text = self._dataArray[index].videoDescription;
...
-(void) openSpecifiedVideo:(uint)index tableView:(UITableView *)tableViewRef {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        NSString* tempFileName = @"/SafetyBox/tempfileName.mp4";
    ...
    //    if([NSFileManager.defaultManager fileExistsAtPath:dataPath isDirectory:false])
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            AVPlayerViewController *mediaPlayer = [[AVPlayerViewController alloc] init];
            [self configureMediaPlayer:&mediaPlayer]; //Configure the Media Player
            
            //Set the Media of the Player
            mediaPlayer.player = [[AVPlayer alloc] initWithURL:[NSURL fileURLWithPath:dataPath]];
            mediaPlayer.player.allowsExternalPlayback = true;
            
            [self._parentController presentViewController:mediaPlayer animated:true completion:^(){
...
-(void) configureMediaPlayer:(AVPlayerViewController **) mediaMngr{
    (*mediaMngr).showsPlaybackControls = true;
    (*mediaMngr).exitsFullScreenWhenPlaybackEnds = false;
    (*mediaMngr).entersFullScreenWhenPlaybackBegins = true;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    [tableView deselectRowAtIndexPath:indexPath animated:true];
    
    [self openSpecifiedVideo:indexPath.row tableView:tableView];
}
